...
/// fields (Title, Authors, Price, Pages, Publisher, Language, Genres) and that the MinPrice and MaxPrice properties, if provided, are
...
public sealed class BookQueryValidator : AbstractValidator<BookQueryDto>
{
    ...
    /// a list of allowed sort fields (Title, Authors, Price, Pages, Publisher, Language, Genres) and uses a case-insensitive string comaprer
    ...
    public static readonly HashSet<string> AllowedSortFields = 
        new ([
            "Title", "Authors", "Price", "Pages", "Publisher", "Language", "Genres"
        ], StringComparer.OrdinalIgnoreCase);

    /// <summary>
    /// Defines the validation rules for the BookQueryDto properties.
    /// </summary>
    public void BookQueryDtoValidator()
    {
        RuleFor(x => x.SortBy)
            .Must(sortBy => string.IsNullOrWhiteSpace(sortBy) || AllowedSortFields.Contains(sortBy))
            .WithMessage("SortBy must be one of the following: Title, Authors, Price, Pages, Publisher, Language, Genres.");
