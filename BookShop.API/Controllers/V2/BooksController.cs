...
public class BooksController(IBookService service) : ControllerBase
{
    ...
    /// <summary>
    /// Retrieves a paginated collection of available books that match the specified filtering and sorting criteria defined in the 
    ...
    /// </returns>
    /// <response code="200">
    /// The request was successful, and a paginated collection of availablebooks that match the specified criteria is returned. 
    /// If no books match the criteria, an empty collection is returned with HTTP 200 status code.
    /// </response>
    /// <response code="400">
    ...
    /// - The user is authenticated but does not have the required role to access this endpoint.
    /// </response>
    [HttpGet("sorted-filtered-books/available")]
    [MapToApiVersion("2.0")]
    [ProducesResponseType(typeof(PageResultDto<BookDto>), StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    [ProducesResponseType(StatusCodes.Status401Unauthorized)]
